...
Copy `SKILL.md` into the agent's system prompt, skill registry, or reusable
instruction area. Keep the `references/`, `templates/`, and `prompts/` folders
available when the agent can read local files.
...
Use `SKILL.md` as the skill instruction file. Keep the repository folders
beside it so Claude can load references and templates as needed.
